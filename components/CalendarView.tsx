...
// Add Briefcase to the import from lucide-react
...
// Helper to replace startOfWeek from date-fns
...
// FIX: Add helper for startOfMonth as it's not exported in the user's environment.
...
export const CalendarView: React.FC<CalendarViewProps> = ({ 
  ...
  const [listScope, setListScope] = useState<'week' | 'all'>('week'); // Scope for list view
  const [now, setNow] = useState(new Date());
  const [slotHeight, setSlotHeight] = useState(64); // px per hour
  ...
  // Effect to handle pending appointment context
  useEffect(() => {
    if (pendingContext) {
      ...
      onClearPendingContext(); // Clear context after use
    }
  ...
  const visibleDays = useMemo(() => {
    ...
    switch (viewMode) {
      case 'month':
        // FIX: Use helper function instead of direct import.
  ...
  const sortedAppointments = useMemo(() => {
    ...
    if (searchText) {
        ...
        result = result.filter(apt => 
            ...
            apt.notes?.toLowerCase().includes(lowerSearch) // Search in notes as well
        );
    }

    // Filter by Scope for List View
    ...
    if (sortConfig) {
        result.sort((a, b) => {
            ...
            switch (sortConfig.key) {
                ...
                case 'statusName':
                    ...
                    break;
                default:
                    // Fallback to default sorting if key is not matched
                    return 0;
            }
        ...
        // Default sort for all views (date/time asc)
        result.sort((a, b) => {
  ...
  const handleDrop = (e: React.DragEvent, day: Date, hour: number) => {
      ...
      if (apt && onAppointmentDrop) {
          onAppointmentDrop(apt, day, `${hour.toString().padStart(2, '0')}:00`);
      }
      setDraggedAptId(null);
  };
  const handleResizeStart = (e: React.MouseEvent, apt: Appointment, type: 'top' | 'bottom') => {
      e.preventDefault(); e.stopPropagation(); 
      setResizingAptId(apt.id);
      setResizeType(type);
      setResizeInitialY(e.clientY);
      setResizeInitialDuration(apt.durationMinutes);
      setResizeInitialStartTime(apt.startTime);
      setTempResizeHeight((apt.durationMinutes / 60) * slotHeight);
      setTempResizeTop(0);
  };
  const handleSlotClick = (day: Date, time: string) => {
      if (skipNextClick.current) return;
      onAddAppointment(day, time);
  };
             ...
             <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex gap-2 w-full sm:w-auto">
                    <div className="relative flex-1 sm:w-64">
                        <input 
                            type="text" 
                            placeholder="Buscar (cliente, tratamiento, personal, estado, notas)..." 
                            value={searchText}
                            onChange={e => setSearchText(e.target.value)}
                            className="w-full pl-9 pr-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white"
                        />
                    ...
                    <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-lg shrink-0">
                        <button 
                            ...
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${listScope === 'week' ? 'bg-white dark:bg-gray-600 shadow text-teal-600' : 'text-gray-700 dark:text-gray-500'}`}
                        >
                            Esta Semana
                        </button>
                        <button 
                            onClick={() => setListScope('all')}
                            className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${listScope === 'all' ? 'bg-white dark:bg-gray-600 shadow text-teal-600' : 'text-gray-700 dark:text-gray-500'}`}
                        >
                            Todo
                        </button>
                    </div>
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700/50 sticky top-0 z-10">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('date')}>
                                <div className="flex items-center">Fecha / Hora {getSortIndicator('date')}</div>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('clientName')}>
                                <div className="flex items-center">Cliente {getSortIndicator('clientName')}</div>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('serviceName')}>
                                <div className="flex items-center">Tratamiento {getSortIndicator('serviceName')}</div>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('staffName')}>
                                <div className="flex items-center">Personal {getSortIndicator('staffName')}</div>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('price')}>
                                <div className="flex items-center">Precio {getSortIndicator('price')}</div>
                            </th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase cursor-pointer hover:text-gray-800 dark:hover:text-gray-200" onClick={() => requestSort('statusName')}>
                                <div className="flex items-center">Estado {getSortIndicator('statusName')}</div>
                            </th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {currentItems.map(apt => {
                            ...
                            const bgClass = service?.color.split(' ')[0] || 'bg-gray-100';
                            const textClass = service?.color.split(' ')[1] || 'text-gray-800';

                            return (
                                <tr key={apt.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm font-medium text-gray-900 dark:text-white">{format(new Date(apt.date), 'dd MMM yyyy', {locale: es})}</div>
                                        <div className="text-sm text-gray-600 dark:text-gray-400">{apt.startTime}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white font-medium">
                                        <button onClick={() => onViewClient(apt.clientId)} className="hover:underline hover:text-teal-600">
                                            {apt.clientName}
                                        </button>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {staffMember ? (
                                            <div className="flex items-center">
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-2 ${staffMember.color}`}>
                                                    {staffMember.name.charAt(0)}
                                                </div>
                                                <span className="text-sm text-gray-700 dark:text-gray-300">{apt.staffName}</span>
                                            </div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${status?.color || 'bg-gray-100 text-gray-700'}`}>
                                            {apt.statusName}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex items-center justify-end space-x-3">
                                            {!status?.isBillable && onQuickComplete && (
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); onQuickComplete(apt); }}
                                            ...
                                            <button onClick={() => onEditAppointment(apt)} className="text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-200">
                                                Editar
                                            </button>
                                        </div>
                                    </td>
                        ...
                        {currentItems.length === 0 && (
                            <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-500">No se encontraron citas.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {totalPages > 1 && (
                <div className="px-6 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-800">
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                        Mostrando <span className="font-medium">{indexOfFirstItem + 1}</span> a <span className="font-medium">{Math.min(indexOfLastItem, sortedAppointments.length)}</span> de <span className="font-medium">{sortedAppointments.length}</span>
                    </div>
                    <div className="flex space-x-2">
                        <button 
                            onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                            disabled={currentPage === 1}
                            className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-md text-sm disabled:opacity-50 hover:bg-white dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"
                        >
                            Anterior
                        </button>
                        <button 
                      ...
                      <div key={i} className="grid grid-cols-7 h-full">
                          {week.map(day => {
                              ...
                              return (
                                  <div 
                                      key={day.toISOString()} 
                                      className={`border-b border-r border-gray-100 dark:border-gray-700 p-1 min-h-[80px] relative hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors ${!isCurrMonth ? 'bg-gray-50/50 dark:bg-gray-800/50 text-gray-600 dark:text-gray-400' : ''}`}
                                      onClick={() => onAddAppointment(day)}
                                      onDragOver={(e) => { e.preventDefault(); }}
                                      onDrop={(e) => handleDrop(e, day, 9)}
                                  >
                                      <div className={`text-xs font-medium mb-1 ${isToday ? 'bg-teal-600 text-white w-6 h-6 flex items-center justify-center rounded-full' : 'text-gray-800 dark:text-gray-200'}`}>
                                          {format(day, 'd')}
                                      </div>
                                          ...
                                          {dayApts.map(apt => {
                                              ...
                                              return (
                                                  <div 
                                                      ...
                                                      onClick={(e) => { e.stopPropagation(); if (skipNextClick.current) return; onEditAppointment(apt); }}
                                                      className={`text-[10px] px-1.5 py-0.5 rounded truncate cursor-pointer shadow-sm border-l-2 ${service?.color.split(' ')[0]} ${service?.color.split(' ')[1]} ${status?.isBillable ? 'border-l-emerald-500' : 'border-l-transparent'}`}
                                                  >
                    ...
                    <div className={`grid w-full relative ${viewMode === 'day' ? 'grid-cols-1' : (showWeekends ? 'grid-cols-7' : 'grid-cols-5')}`}>
                        {nowTop !== null && (
                            <div className="absolute left-0 right-0 z-30 pointer-events-none flex items-center" style={{ top: nowTop }}>
                                <div className="w-full border-t-2 border-red-500 opacity-60"></div>
                            ...
                            return (
                                <div key={day.toISOString()} className="relative border-l border-gray-100 dark:border-gray-700 group">
                                    ...
                                    {dayAppointments.map(apt => {
                                        ...
                                        const height = isResizing && tempResizeHeight ? tempResizeHeight : (apt.durationMinutes / 60) * slotHeight;
                                        ...
                                        return (
                                            <div
                                                ...
                                                onDragStart={(e) => handleDragStart(e, apt)}
                                                onClick={(e) => { e.stopPropagation(); if (skipNextClick.current) return; onEditAppointment(apt); }}
                                                className={`absolute inset-x-1 rounded-md px-2 py-1 shadow-sm border-l-[3px] cursor-pointer z-10 overflow-hidden transition-shadow
                                                    ${service?.color.split(' ')[0] || 'bg-gray-100'} 
                                                    ${service?.color.split(' ')[1] || 'text-gray-800'} 
                                                    ${status?.isBillable ? 'ring-1 ring-emerald-400 ring-opacity-50' : ''}
                                                    ${isResizing ? 'shadow-lg z-50 opacity-90 scale-[1.02]' : 'hover:shadow-md'}
                                                `}
                                                style={{ top: visualTop, height: Math.max(height, 28), cursor: isResizing ? 'row-resize' : 'grab' }}
                                            >
                                                ...
                                                <div className="flex justify-between items-start text-xs pointer-events-none">
                                                    <span className="font-bold">{apt.startTime}</span>
                                                    {status?.isBillable && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>}
                                                </div>
                                                <div className="font-bold text-xs truncate leading-tight mt-0.5 pointer-events-none">
                                                    {getClientName(apt.clientId)}
                                                </div>
                                                ...
                                                {staffMember && (
                                                    <div className="absolute bottom-1 right-1">
                                                        <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold ${staffMember.color}`}>
                                                            {staffMember.name.charAt(0)}
                                                        </div>
                                                    </div>
                                                )}
            ...
            return (
                <div key={day.toISOString()} className={`flex gap-3 ${dayAppointments.length === 0 ? 'opacity-60' : ''}`}>
                    <div className="w-14 shrink-0 flex flex-col items-center pt-1">
                        ...
                        <div className={`mt-1 w-8 h-8 flex items-center justify-center rounded-full text-lg font-medium ${isToday ? 'bg-teal-600 text-white' : 'text-gray-900 dark:text-gray-200 bg-white dark:bg-gray-700'}`}>
                            {format(day, 'd')}
                        </div>
                    </div>
                    <div className="flex-1 min-w-0 pt-1">
                        {dayAppointments.length > 0 ? (
                            ...
                            <div className="h-full pt-2" onClick={() => onAddAppointment(day)}>
                                <div className="border-t-2 border-dashed border-gray-200 dark:border-gray-700 w-full" />
                            </div>
                        )}
                    </div>
                </div>
    ...
    <div className="flex flex-col h-full space-y-3 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
          <div>
            <div className="flex items-center gap-2">
                 ...
                 <div className="hidden md:flex items-center bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-0.5 ml-4">
                     ...
                     <span className="px-3 text-sm font-medium text-gray-700 dark:text-gray-200 min-w-[140px] text-center capitalize">
                         {viewMode === 'day' 
                            ? format(currentDate, 'd MMMM yyyy', {locale: es}) 
                            : viewMode === 'month'
                                ? format(currentDate, 'MMMM yyyy', {locale: es})
                                : `${format(startDate, 'd MMM')} - ${format(endDate, 'd MMM', {locale: es})}`
                         }
                     </span>
          ...
          <div className="hidden md:flex items-center gap-2">
               <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    {([['list', 'Lista', ListIcon], ['day', 'Día', Clock], ['week', 'Semana', LayoutGrid], ['month', 'Mes', CalendarIcon]] as const).map(([mode, label, Icon]) => (
                        <button 
                            ...
                            className={`flex items-center px-3 py-1.5 text-sm font-medium rounded-md transition-all ${viewMode === mode ? 'bg-white dark:bg-gray-700 text-teal-600 shadow-sm' : 'text-gray-700 hover:text-gray-900 dark:hover:text-gray-300'}`}
                        >
               ...
               <button onClick={() => onAddAppointment()} className="bg-teal-600 text-white px-4 py-2 rounded-lg flex items-center hover:bg-teal-700 shadow-sm transition-colors">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            ...
            {statuses.map(status => (
                <button 
                  ...
                  className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${selectedStatuses.includes(status.id) ? 'bg-teal-50 border-teal-500 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300'}`}
                >
                    {status.name}
                </button>
            ))}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
            ...
            {services.map(service => (
                <button
                    ...
                    className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${selectedTreatments.includes(service.id) ? 'bg-blue-50 border-blue-500 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300'}`}
                >
                    {service.name}
                </button>
            ))}
      ...
      <button 
        ...
        className="md:hidden fixed bottom-20 right-4 w-14 h-14 bg-teal-600 text-white rounded-full shadow-lg flex items-center justify-center z-50 active:scale-95 transition-transform"
      >
